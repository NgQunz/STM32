...
void LCD_Send_Hex(uint8_t num)
{
    ...
    // Hi?n th? ti?n t? "0x" n?u c?n
    LCD_Data_Write('0');
    LCD_Data_Write('x');

    // L?y 4 bit cao và chuy?n thành ký t? hex
    temp = num >> 4;
    if(temp > 9) {
        temp += 'A' - 10; // Chuy?n 10-15 thành 'A'-'F'
    } else {
        temp += '0'; // Chuy?n 0-9 thành '0'-'9'
    }
    LCD_Data_Write(temp); // Hi?n th? ký t? d?u tiên

    // L?y 4 bit th?p và chuy?n thành ký t? hex
    temp = num & 0x0F;
    if(temp > 9) {
        temp += 'A' - 10; // Chuy?n 10-15 thành 'A'-'F'
    } else {
    ...
    LCD_Data_Write(temp); // Hi?n th? ký t? th? hai
}
