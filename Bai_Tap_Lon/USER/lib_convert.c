...
int16_t Convert_14bit_To_Int16(uint8_t msb, uint8_t lsb)
{
    ...
    return value >> 2; // Right shift by 2 to convert to 14-bit value
}

float Convert_14bit_To_Float(uint8_t msb, uint8_t lsb)
{
    int16_t raw_value = Convert_14bit_To_Int16(msb, lsb);
    return (float)raw_value / 4096.0f; // Convert to g (assuming ±2g range)
}
