...
uint8_t I2C2_Scan_Address(void) {
    ...
    for(address = 0x20; address <= 0x3F; address++) {
        ...
        if(I2C2->SR1 & I2C_SR1_ADDR) {
            ...
            USART1_Send_Hex(address);  // Changed to hex display
            USART1_Send_String("\r\n");
            found = address;
            break;
        }
    ...
    if(!found) {
        ...
        USART1_Send_Hex(0x27);  // Changed to hex display
        USART1_Send_String("\r\n");
        return 0x27;
    }
    return found;
...
void LCD_Send_Hex(uint8_t num)
{
    ...
    // Lay 4 bit cao và chuyen thành ký tu hex
    temp = num >> 4;
    if(temp > 9) {
        temp += 'A' - 10; // Chuyen 10-15 thành 'A'-'F'
    } else {
        temp += '0'; // Chuyen 0-9 thành '0'-'9'
    }
    LCD_Data_Write(temp); // Hien thi ký tu dau tiên

    // Lay 4 bit thap và chuyen thành ký tu hex
    temp = num & 0x0F;
    if(temp > 9) {
        temp += 'A' - 10; // Chuyen 10-15 thành 'A'-'F'
    } else {
    ...
    LCD_Data_Write(temp); // Hien thi ký tu thu hai
}
