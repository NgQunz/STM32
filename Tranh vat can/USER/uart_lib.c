...
void USART1_Init(uint32_t baud_rates){
	...
	/* USART1 Pin */
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);		/* clock ngoai vi GPIO */
	pin.GPIO_Mode = GPIO_Mode_AF_PP;												/* TX pin */
	pin.GPIO_Pin = GPIO_Pin_9;
	pin.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA,&pin);
	
	pin.GPIO_Mode = GPIO_Mode_IN_FLOATING;									/* RX pin */
...
void USART2_Init(uint32_t baud_rates){
	...
	/* TX USART2 */
	RCC_APB2PeriphClockCmd(RCC_APB1Periph_USART2,ENABLE);		/* clock ngoai vi USART */
	usart.USART_BaudRate = baud_rates;											/* toc do truyen: baud_rates */
	usart.USART_Mode = USART_Mode_Tx | USART_Mode_Rx;
	usart.USART_Parity = USART_Parity_No;
	usart.USART_StopBits = USART_StopBits_1;
	...
	USART_Cmd(USART2,ENABLE);																/* cho phep USART2 hoat dong */
	
	USART_ITConfig(USART2,USART_IT_RXNE,ENABLE);						/* ngat nhan USART2 					*/
	NVIC_EnableIRQ(USART2_IRQn);
}
void USART3_Init(uint32_t baud_rates){
	GPIO_InitTypeDef pin;
	USART_InitTypeDef usart;
	
	/* USART3 Pin */
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);		/* clock ngoai vi GPIO */
	pin.GPIO_Mode = GPIO_Mode_AF_PP;												/* TX pin */
	pin.GPIO_Pin = GPIO_Pin_10;
	pin.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,&pin);
	...
	pin.GPIO_Mode = GPIO_Mode_IN_FLOATING;									/* RX pin */
	pin.GPIO_Pin = GPIO_Pin_11;
	pin.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,&pin);
	
	/* TX USART3 */
	RCC_APB2PeriphClockCmd(RCC_APB1Periph_USART3,ENABLE);		/* clock ngoai vi USART */
	usart.USART_BaudRate = baud_rates;											/* toc do truyen: baud_rates */
	usart.USART_Mode = USART_Mode_Tx | USART_Mode_Rx;
	usart.USART_Parity = USART_Parity_No;
	usart.USART_StopBits = USART_StopBits_1;
	...
	USART_Cmd(USART3,ENABLE);																/* cho phep USART1 hoat dong */
	
	USART_ITConfig(USART3,USART_IT_RXNE,ENABLE);						/* ngat nhan USART3 					*/
