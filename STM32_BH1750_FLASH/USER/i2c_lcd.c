...
uint8_t I2C2_Scan_Address(void) {
    uint8_t address;
    uint8_t found = 0;
    printf_string("Scanning I2C2 LCD addresses...\r\n");
    ...
    for(address = 0x20; address <= 0x3F; address++) {
        ...
        if(I2C2->SR1 & I2C_SR1_ADDR) {
            (void)I2C2->SR2;
            printf_string("LCD found at address: 0x");
            printf_num(address);  // Changed to hex display
            printf_string("\r\n");
            found = address;
            break;
        }
    ...
    if(!found) {
        printf_string("LCD not found, using default address: 0x");
        printf_num(0x27);  // Changed to hex display
        printf_string("\r\n");
        return 0x27;
    }
    return found;
...
void LCD_Send_Hex(uint8_t num)
{
	...
	// Hien thi toan tu 0x neu can
	LCD_Data_Write('0');
	LCD_Data_Write('x');

	// Lay 4 bit cao v� chuyen th�nh k� tu hex
	temp = num >> 4;
	if (temp > 9)
	{
		temp += 'A' - 10; // Chuyen 10-15 th�nh 'A'-'F'
	}
	else
	{
		temp += '0'; // Chuyen 0-9 th�nh '0'-'9'
	}
	LCD_Data_Write(temp); // Hien thi k� tu dau ti�n

	// Lay 4 bit thap v� chuyen th�nh k� tu hex
	temp = num & 0x0F;
	if (temp > 9)
	{
		temp += 'A' - 10; // Chuyen 10-15 th�nh 'A'-'F'
	...
	LCD_Data_Write(temp); // Hien thi k� tu thu hai
}
